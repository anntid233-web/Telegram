...
/**
 * Экран "Кастомизация" — полупрозрачные удалёнки, метка удалёнок,
 * ссылки на "Навигация в приложении" и "Pill Stack", полезные функции.
 */
public class CustomizationSettingsActivity extends BaseFragment {
    ...
    public View createView(Context context) {
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == translucentDeletedRow) {
                ...
                // показать диалог выбора метки: Убрать значок / Корзинка / Крестик / Глазик
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public boolean isEnabled(RecyclerView.ViewHolder holder) {
            ...
            return position != headerRow && position != shadow1Row
                    && position != shadow2Row && position != usefulHeaderRow;
        }
            ...
            switch (holder.getItemViewType()) {
                ...
                case 2: {
                    ...
                    if (position == deletedLabelRow) {
                        cell.setTextAndValue("Метка удалёнок", "Корзинка", true);
                    } else if (position == navigationRow) {
                        cell.setTextAndValueAndIcon("Навигация в приложении",
                                "Настройки верхнего меню и боковой панели",
                                true, R.drawable.msg_recent, true);
                    } else if (position == pillStackRow) {
                        cell.setTextAndValueAndIcon("Pill Stack",
                                "Интерактивные виджеты в поле поиска",
                                true, R.drawable.msg_search, false);
                    }
                    break;
                }
            }
        ...
        public int getItemViewType(int position) {
            if (position == headerRow || position == usefulHeaderRow) {
                ...
                return 1;
            } else if (position == deletedLabelRow || position == navigationRow || position == pillStackRow) {
                return 2;
            } else {
                return 3;
            }
