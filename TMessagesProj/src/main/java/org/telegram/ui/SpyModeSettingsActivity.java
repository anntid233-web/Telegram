    ...
    public View createView(Context context) {
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == saveDeletedRow) {
                ...
                // TODO: открыть выбор папки для вложений
            } else if (position == exportDbRow) {
                // TODO: экспорт локальной базы (ACTION_CREATE_DOCUMENT)
            } else if (position == importDbRow) {
                // TODO: импорт локальной базы (ACTION_OPEN_DOCUMENT)
            } else if (position == clearRow) {
                // TODO: диалог подтверждения очистки локальной базы шпиона
            }
        });
            ...
            switch (holder.getItemViewType()) {
                ...
                case 1: {
                    ...
                    if (position == saveDeletedRow) {
                        ...
                        cell.setTextAndCheck("Сохранять вложения", pref("spy_save_attachments", true), true);
                    }
                    break;
                }
                case 2: {
